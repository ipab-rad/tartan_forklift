"""Module for parsing and validating YAML configurations."""
...
class ConfigParser:
    """Class to parse and validate a YAML configuration file."""
    ...
    def load_config(self, config_path) -> Parameters:
        ...
        with open(config_path) as file:
            ...
            parameters = Parameters(
                ...
                local_rosbags_directory=yaml_config.get(
                    'local_rosbags_directory'
                ),
                ...
                cloud_upload_directory=yaml_config.get(
                    'cloud_upload_directory'
                ),
                mcap_bin_path=yaml_config.get('mcap_bin_path'),
                mcap_compression_chunk_size=int(
                    yaml_config.get('mcap_compression_chunk_size')
                ),
                compression_parallel_workers=int(
                    yaml_config.get('compression_parallel_workers')
                ),
                compression_queue_max_size=int(
                    yaml_config.get('compression_queue_max_size')
                ),
            )
            return parameters
    ...
    def validate_yaml_config(self, yaml_config: dict) -> None:
        """
        Validate the provided YAML configuration dictionary.
        ...
        """
        ssh_alias_defined = bool(
            yaml_config.get('cloud_ssh_alias', '').strip()
        )
        ...
        missing_keys = [
            key for key in self.required_params if key not in yaml_config
        ]
        if missing_keys:
            raise KeyError(
        ...
        if (
            ...
            raise ValueError(
                '"local_host_user" must be a string and non-empty.'
            )
        ...
        if not os.path.isabs(yaml_config['local_rosbags_directory']):
            raise ValueError(
                '"local_rosbags_directory" must be an absolute path.'
            )
        ...
        if not ssh_alias_defined:
            if (
                ...
                raise ValueError(
                    '"cloud_user" must be a string and non-empty '
                    'if "cloud_ssh_alias" is not defined.'
                )

            if not yaml_config['cloud_hostname'].strip():
                raise ValueError(
                    '"cloud_hostname" must be non-empty '
                    'if "cloud_ssh_alias" is not defined.'
                )

        if not os.path.isabs(yaml_config['cloud_upload_directory']):
            raise ValueError(
                '"cloud_upload_directory" must be an absolute path.'
            )
        ...
        if (
            ...
            raise ValueError(
                '"compression_parallel_workers" must be a positive integer.'
            )
        ...
        if (
            ...
            raise ValueError(
                '"compression_queue_max_size" must be a positive integer.'
            )
